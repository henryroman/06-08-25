...
function validatePayload(body: unknown): any {
  ...
  return {
    ...
    paymentMode: payload.paymentMode || 'pay_later'
  };
}
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!raw) return new Response(JSON.stringify({ success: false, message: 'Invalid JSON body' }), {
    ...
    if (!process.env.NOTION_TOKEN || !process.env.NOTION_APPOINTMENTS_DB_ID) {
      return new Response(JSON.stringify({ success: true, simulated: true, appointmentId: `sim_${Date.now()}` }), {
    ...
    if (!available) {
      return new Response(JSON.stringify({ success: false, message: 'Selected slot not available' }), {
    ...
    try {
      ...
      const appointmentInput = {
        ...
        Status: booking.paymentMode === 'pay_later' ? 'confirmed' : 'pending_payment',
        Notes: booking.notes ?? '',
        Email: booking.email
      };
